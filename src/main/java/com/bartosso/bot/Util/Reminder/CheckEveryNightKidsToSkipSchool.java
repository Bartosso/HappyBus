...
@Slf4j
public class CheckEveryNightKidsToSkipSchool extends TimerTask {
    ...
    public void run() {
        ...
        if (sickLeaves!=null){
        List<SickLeave> endedSick  = sickLeaves.stream().filter(sickLeave -> sickLeave.getSickDate().isBefore(localDate)).collect(toList());
        if (endedSick != null) {
          endedSick.forEach(this::returnKidInTheBus);
        }
        List<SickLeave> todaySickLeaves = sickLeaves.stream().filter(sickLeave -> sickLeave.getSickDate().getDayOfYear()==localDate.getDayOfYear())
                .collect(toList());
